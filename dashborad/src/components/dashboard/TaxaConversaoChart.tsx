...
export function TaxaConversaoChart() {
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      try {
        ...
        const conversion: number[] = json.conversion || [];
        const totals: number[] = json.total_counts || [];
        const convertedCounts: number[] = json.converted_counts || [];

        setLabels(labels);

        // If there are no leads at all (totals all zero), show zeroed series
        ...
        setCounts(convertedCounts);
        setData(labels.map((lbl, i) => ({ name: lbl, taxa: conversion[i] ?? 0, count: convertedCounts[i] ?? 0, total: totals[i] ?? 0 })));
        setLatest(json.conversion_latest ?? 0);
      } catch (e) {
        // ignore
      }
    };
      ...
      <div className="flex items-center justify-between mb-6 relative z-10">
        ...
        <div className="text-right">
          <div className="flex items-center gap-1 justify-end">
            <span className="text-3xl font-bold gradient-text">{latest}%</span>
              ...
              content={({ active, payload, label }) => {
                ...
                return (
                  <div className="p-3 text-sm" style={{ color: 'hsl(var(--foreground))' }}>
                    ...
                    <div>Convertidos: {point.count ?? 0}</div>
